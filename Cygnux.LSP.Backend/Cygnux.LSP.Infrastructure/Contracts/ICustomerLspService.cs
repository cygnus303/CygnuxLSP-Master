...
public interface ICustomerLspService
{
    ...
    Task<IEnumerable<LspTatDetailResponse>> GetLspTatList(Guid customerId, int page, int pageSize,Guid userId, string? customerName, string? lspName, string? product, string? origin, string? destination, int? tat,string? mmodeDescription,decimal? RatePerKg);
    ...
    Task<IEnumerable<LspTatValidationResult>> GetTATdata(string BulkLsp, Guid entryBy);
    Task<CustomerLspTatSpResponse> InsertLspTatData(string bulkLspJson, Guid entryBy);
    Task<UserRoleResponse> GetUserRolesById(Guid customerId);
}
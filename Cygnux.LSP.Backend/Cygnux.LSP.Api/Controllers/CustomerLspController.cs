...
public class CustomerLspController : ControllerBase
{
    ...
    [HttpGet]
    [Route("Tat")]
    public async Task<IActionResult> GetLspTatList([FromQuery] int page, [FromQuery] int pageSize,Guid userId,string customerName,string lspName)
    {
        return Ok(await _customerLspRepository.GetLspTatList(Guid.NewGuid(), page, pageSize, userId,customerName,lspName));
    }
    ...
    [HttpGet]
    public async Task<IActionResult> GetLspMappingList([FromQuery] int page, [FromQuery] int pageSize,Guid userId)
    {
        return Ok(await _customerLspRepository.GetLspMappingList(Guid.NewGuid(), page, pageSize, userId));
    }

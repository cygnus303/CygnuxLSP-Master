...
window.toggleSidebarMenu = toggleSidebarMenu;  // Make it globally accessible
...
window.$toggle_nav_top.on("click", function () {
...
$('.simplebar-wrapper .simplebar-content-wrapper').animate({
    scrollTop: $('.simplebar-wrapper .simplebar-content-wrapper a.active').offset()
}, 1000);
}
...
(() => {
  ...
  pinIcon.forEach((item, index) => {
    ...
    let InitialLocalStorage = JSON.parse(localStorage.getItem("pins") || false);
